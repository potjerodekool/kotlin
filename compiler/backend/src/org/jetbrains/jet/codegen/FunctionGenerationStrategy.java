...
public abstract class FunctionGenerationStrategy<T extends CallableDescriptor> {
    ...
    public static class CodegenBased<T extends CallableDescriptor> extends FunctionGenerationStrategy<T> {
        ...
        public ExpressionCodegen initializeExpressionCodegen(JvmMethodSignature signature, MethodContext context, MethodVisitor mv, Type returnType) {
            assert returnType != null : "Return type can't be null: " + signature;

            return new ExpressionCodegen(mv, getFrameMap(state.getTypeMapper(), context), returnType, context, state);
        }

        public void generateLocalVarNames(ExpressionCodegen codegen) {
