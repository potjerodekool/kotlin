...
public class KotlinCacheManagerUtil {
    ...
    public static KotlinDeclarationsCache getDeclarationsFromProject(@NotNull JetElement element) {
        ...
        return KotlinCacheManager.getInstance(jetFile.getProject()).getDeclarationsFromProject(TargetPlatformDetector.detectPlatform(jetFile));
    }

    @NotNull
    public static BindingContext getHeadersBindingContext(@NotNull JetElement element) {
        JetFile jetFile = (JetFile) element.getContainingFile();
        KotlinDeclarationsCache declarationsCache = KotlinCacheManager.getInstance(jetFile.getProject())
                .getDeclarationsFromProject(TargetPlatformDetector.detectPlatform(jetFile));
